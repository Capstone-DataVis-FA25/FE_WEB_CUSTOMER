...
export const UnifiedProgressBar: React.FC<UnifiedProgressBarProps> = ({
  ...
  useEffect(() => {
    if (!isMinimized) {
      // Set flag that progress bar is open
      localStorage.setItem('progress-bar-open', 'true');
      // Dispatch event to close chatbot
  ...
  // Chatbot closed: 24px (right-6) + 120px (w-30 button) + 12px gap = 156px
  // Chatbot open: 24px (right-6) + 384px (w-96 chatbot) + 12px gap = 420px
  const rightPosition = chatbotOpen ? 'calc(24px + 384px + 12px)' : 'calc(24px + 120px + 12px)';

  return (
    <div className="fixed bottom-6 z-50 w-80" style={{ right: rightPosition }}>
      {/* Minimize button */}
