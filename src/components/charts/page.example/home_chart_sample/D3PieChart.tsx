...
const D3PieChart: React.FC<PieChartProps> = ({
  ...
  useEffect(() => {
    ...
    if (showLegend) {
      arcs.append('text')
        ...
        .style('opacity', 0)
        .text(d => {
          const percentage = ((Number(d.data[valueKey]) / d3.sum(data, d => Number(d[valueKey]))) * 100).toFixed(1);
          return `${percentage}%`;
        })
        .transition()
  ...
  }, [data, width, height, margin, valueKey, labelKey, colors, showLegend, animationDuration, innerRadius, isDarkMode]);
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {data.map((item, index) => {
              ...
              return (
                <div
                  key={label}
                  className="flex items-center gap-2 px-3 py-2 bg-card rounded-lg border border-border hover:shadow-md transition-all duration-200"
                >
                  <div
