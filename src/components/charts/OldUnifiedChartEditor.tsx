...
export interface UnifiedChartEditorProps {
  ...
  initialStructuredConfig?: MainChartConfig;
...
const OldUnifiedChartEditor: React.FC<UnifiedChartEditorProps> = ({
  initialArrayData,
  ...
  const initialConfigRef = useRef<MainChartConfig | null>(null);
  ...
  //   (importData: MainChartConfig | null) => {
  //     try {
  //       if (!importData) {
  //         return;
  //       }
  ...
  //         setChartType(chartTypeToImport as ChartType);
  //       }
  ...
  //                 ((configToImport as )?.disabledLines as string[]) || []
  //               ) as string[],
  //             }
  //           : {}),
  //         ...(targetChartType === 'bar'
  ...
  //       // Create a proper MainChartConfig by merging with defaults
  ...
  useEffect(() => {
    ...
    if (!chartConfig) {
      ...
      chartConfig &&
      (!chartConfig.config.yAxisKeys || chartConfig.config.yAxisKeys.length === 0)
    ) {
      ...
      if (processedInitialData.length > 0 && !chartConfig.config.xAxisKey) {
  ...
  }, [processedInitialData, chartConfig, chartType, initialStructuredConfig, dataset?.headers]); // Dependencies để tránh stale closure
  ...
  const safeChartConfig = useMemo(() => {
    return chartConfig || getDefaultChartConfig(chartType);
  ...
  useEffect(() => {
    if (chartConfig) {
      ...
      const currentYAxisKeys = chartConfig.config.yAxisKeys || [];
  ...
  //   (newConfig: Partial<MainChartConfig>) => {
  ...
  //     const currentConfig = chartConfig || getDefaultChartConfig(chartType);
...
export default OldUnifiedChartEditor;
