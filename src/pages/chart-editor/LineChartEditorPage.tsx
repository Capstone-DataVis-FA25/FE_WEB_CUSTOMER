...
import type { LineChartConfig as ChartConfig, ColorConfig, FormatterConfig } from '@/types/chart';
import { salesData } from '@/components/charts/data/data'; // Import sample data

const LineChartEditorPage: React.FC = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (chartData.datasetId) {
          ...
          console.log('📊 LineChart config extracted from API:', {
            yAxisKeys: config.yAxisKeys,
            xAxisKey: config.xAxisKey,
            title: config.title,
            hasDataset: !!datasetData,
            datasetHeaders: datasetData.headers?.map(h => ({ name: h.name, type: h.type })),
          });
          ...
          const defaultConfig = {
            title: chart?.name || 'Sample Line Chart',
            xAxisLabel: 'Month',
...
export default LineChartEditorPage;
