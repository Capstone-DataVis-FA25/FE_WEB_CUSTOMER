...
import type { AreaChartConfig as ChartConfig, ColorConfig, FormatterConfig } from '@/types/chart';
import { areaArrayData } from '@/components/charts/data/data'; // Import area chart sample data

const AreaChartEditorPage: React.FC = () => {
  ...
  // Chart editor state
  const [chartData, setChartData] = useState<(string | number)[][]>(areaArrayData); // Initialize with sample data
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (chartData.datasetId) {
          ...
          const defaultConfig = {
            title: chart?.name || 'Sample Area Chart',
            xAxisLabel: 'Month',
            yAxisLabel: 'Usage (%)',
            xAxisKey: 'month',
            yAxisKeys: ['mobile', 'desktop', 'tablet'],
            width: 800,
            height: 400,
            showLegend: true,
            showGrid: true,
          ...
          const defaultColors = {
            mobile: { light: '#3b82f6', dark: '#60a5fa' },
            desktop: { light: '#f97316', dark: '#fb923c' },
            tablet: { light: '#10b981', dark: '#34d399' },
          };
...
export default AreaChartEditorPage;
